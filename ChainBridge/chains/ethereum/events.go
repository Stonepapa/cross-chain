...
func (l *listener) handleErc20DepositedEvent(destId msg.ChainId, nonce msg.Nonce) (msg.Message, error) {
	...
	l.log.Info("Handling fungible deposit data", "amount", record.Amount, "receipt address", common.BytesToAddress(record.DestinationRecipientAddress).Hex())
...
func (l *listener) handleErc721DepositedEvent(destId msg.ChainId, nonce msg.Nonce) (msg.Message, error) {
	...
	l.log.Info("Handling fungible deposit data", "token id", record.TokenID, "receipt address", common.BytesToAddress(record.DestinationRecipientAddress).Hex())
